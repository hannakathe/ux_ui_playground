...
function Section({ title, defaultOpen = true, children }: { title: string; defaultOpen?: boolean; children: React.ReactNode }) {
  ...
  return (
    <div className="border-b border-[var(--border)]">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-5 py-3.5 group transition-colors hover:bg-[var(--surface-2)]">
        <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-indigo-400/80 group-hover:text-indigo-400 transition-colors">{title}</span>
...
function SliderWithInput({ label, value, min = 0, max = 100, step = 1, unit = "px", onChange }: { label: string; value: number; min?: number; max?: number; step?: number; unit?: string; onChange: (v: number) => void }) {
  return (
    <div className="space-y-2.5">
      <div className="flex items-center justify-between">
        <label className="text-xs text-[var(--fg)] font-medium">{label}</label>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2.5 w-full h-9 px-3 rounded-lg bg-[var(--surface-2)] border border-[var(--border)] text-xs hover:border-[var(--border-light)] transition-colors">
          <div className="w-5 h-5 rounded-md border border-[var(--border)]" style={{ backgroundColor: value }} />
          <span className="font-mono text-[var(--fg)]">{value}</span>
        </button>
        {open && (
          <>
            <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
            <div className="absolute top-full left-0 mt-2 z-50 bg-[var(--surface)] rounded-xl p-3 border border-[var(--border)] shadow-2xl shadow-black/40">
              <HexColorPicker color={value} onChange={onChange} />
              <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="mt-2 w-full h-8 px-3 rounded-lg bg-[var(--surface-2)] border border-[var(--border)] text-xs font-mono text-[var(--fg)] focus:outline-none focus:border-indigo-500" />
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--surface)] overflow-hidden">
      <div className="px-5 py-4 border-b border-[var(--border)]">
        ...
        <p className="text-[11px] text-[var(--muted)] mt-1">Adjust element styles in real time</p>
      </div>
      <div className="flex-1 overflow-y-auto">
        ...
        <Section title="Layout" defaultOpen={false}>
          <div className="space-y-2">
            <label className="text-xs text-[var(--fg)] font-medium">Display</label>
          ...
          {(styles.display === "flex" || styles.display === "inline-flex") && (
            <>
              <div className="space-y-2">
                <label className="text-xs text-[var(--fg)] font-medium">Direction</label>
              ...
              <div className="space-y-2">
                <label className="text-xs text-[var(--fg)] font-medium">Justify Content</label>
              ...
              <div className="space-y-2">
                <label className="text-xs text-[var(--fg)] font-medium">Align Items</label>
